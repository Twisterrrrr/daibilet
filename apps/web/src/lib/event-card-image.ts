...
import { isComboSessionTitle } from '@/lib/home-showcase-sections';
 ...
 * Combo tickets often share one supplier cover - prefer the venue photo instead.
 */
export function resolveEventCardPrimaryImage(session: EventCardImageSource): string | null {
  ...
  if (isComboSessionTitle(session.title) && session.venueSlug) {
    const venueHero = resolveVenueHeroImage(session.venueSlug);
    if (venueHero) return venueHero;
  }
