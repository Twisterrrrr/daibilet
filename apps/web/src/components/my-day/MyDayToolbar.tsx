 ...
 * Scroll-down collapses to a compact strip on all viewports; expand via tap or near top.
 */
    ...
    <div className={className.trim() || undefined} data-my-day-toolbar-wrap>
      ...
      */}
      <div
        ...
        className={`sticky top-[calc(var(--site-header-height)+0.35rem)] z-20 [container-type:inline-size] rounded-2xl border border-slate-200/90 bg-white shadow-sm lg:top-[calc(var(--site-header-height)+0.5rem)] ${
          collapsed ? 'px-3 py-1.5' : 'p-4 sm:p-5'
        }`}
      >
        {/* Compact strip after scroll-down (all viewports). Tap label to expand. */}
        {collapsed ? (
          <div
            data-my-day-toolbar-compact
            className="flex w-full min-h-10 items-center gap-2"
          >
            <button
              type="button"
              onClick={expand}
            ...
            <button
              ...
              data-day-clear
              className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:bg-slate-50 hover:text-slate-800 disabled:opacity-40"
            >
        ...
        {/* Full chrome: hidden while collapsed on every breakpoint. */}
        <div className={collapsed ? 'hidden' : undefined} data-my-day-toolbar-full>
