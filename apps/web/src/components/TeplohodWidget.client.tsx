...
function mountTeplohodWidgetContainers() {
  ...
  try {
    ...
    if (typeof tickets.widget?.init === 'function') {
    ...
    if (typeof tickets.init === 'function') {
...
function containerHasWidgetMarkup(container: HTMLElement | null) {
  ...
  return Boolean(
    ...
    container.querySelector('.ti-tickets-event-tickets-buy, .ti-tickets-widget, iframe'),
...
export function ensureTeplohodWidgetScript() {
  ...
  const afterScriptReady = () => {
    ...
    window.setTimeout(() => mountTeplohodWidgetContainers(), 0);
...
export function TeplohodWidgetEmbed({
  ...
  React.useEffect(() => {
    ...
    const waitForMarkup = (attempt = 0) => {
      ...
      if (containerHasWidgetMarkup(containerRef.current)) {
    ...
    const tryMount = (attempt = 0) => {
      ...
      if (mountTeplohodWidgetContainers()) {
