...
export async function GET(request: Request) {
  ...
  try {
    const payload = await buildPublicDestinationsDto(forceRefresh);
    return publicJsonResponse(payload);
  } catch (error) {
    return publicJsonResponse(
      { error: 'internal_error', message: error instanceof Error ? error.message : 'Failed to load destinations' },
      { status: 500 },
    );
  }
}
