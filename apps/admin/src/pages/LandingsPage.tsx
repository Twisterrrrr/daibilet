...
function landingStatusLabel(status?: string | null) {
...
export function LandingsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Лендинги"
        description="SEO-выборки каталога: правила отбора, покрытие событиями и быстрый контроль попадания карточек."
        meta={
          <>
            <Badge variant="outline">{formatNumber(payload.total)} правил</Badge>
            <Badge variant="outline">{formatNumber(payload.metrics.ready)} готово</Badge>
            {isLoading ? (
      ...
      <div className="mb-4 grid gap-3 md:grid-cols-4">
        ...
        <MetricCard value={payload.metrics.seed} label="Черновики" />
      ...
      <div className="mb-4 flex flex-wrap gap-3 rounded-lg border border-border bg-card p-3">
        ...
        <select className="h-9 rounded-md border border-input bg-background px-3 text-sm" value={status} onChange={(event) => setStatus(event.target.value)}>
          ...
          <option value="seed">Черновик</option>
      ...
      {loadError ? (
        <div className="mb-4">
          <InfoNote>Резерв API: {loadError}</InfoNote>
        </div>
      ) : null}
          ...
          {rows.map((landing) => (
            <tr key={landing.slug} className="border-b border-border last:border-0 hover:bg-secondary/40">
              ...
              <td className="px-4 py-3 align-top">
                <StatusBadge status={landing.status === 'ready' ? 'live' : landing.status === 'seed' ? 'incomplete' : 'error'} label={landingStatusLabel(landing.status)} />
              </td>
              ...
              <td className="px-4 py-3 align-top text-xs text-muted-foreground">
                <div>{landing.seo?.isIndexable ? 'индекс' : 'без индекса'}</div>
  ...
  return (
    <Card className="border-border p-4">
      ...
      <div className="mb-4 flex flex-wrap gap-1.5">
        ...
        {landing.city ? <Badge variant="outline">город: {landing.city}</Badge> : null}
        {landing.venue ? <Badge variant="outline">площадка: {landing.venue}</Badge> : null}
      </div>

      <div className="mb-4 grid gap-3 md:grid-cols-3">
        <RuleBox title="Ключевые слова" items={landing.keywords || []} empty="нет ключевых слов" />
        <RuleBox title="Обязательные теги" items={landing.requiredTags || []} empty="нет тегов" />
        <RuleBox title="Исключения" items={landing.excludedTags || []} empty="нет исключений" />
      </div>

      <div className="mb-4 rounded-lg border border-border bg-secondary/20 p-3">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <div className="text-xs font-medium text-muted-foreground">SEO / переопределение контента</div>
            <div className="text-xs text-muted-foreground">Лендинг и SEO-мета публичной страницы.</div>
          </div>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <Input value={form.title} onChange={(event) => setField('title', event.target.value)} placeholder="Заголовок" className="h-9" />
          <Input value={form.subtitle} onChange={(event) => setField('subtitle', event.target.value)} placeholder="Подзаголовок" className="h-9" />
          ...
          <Input value={form.canonicalUrl} onChange={(event) => setField('canonicalUrl', event.target.value)} placeholder="Канонический URL" className="h-9" />
      ...
      <DataTableShell columns={['Событие', 'Город / место', 'Сеанс', 'Цена', 'Готовность']}>
        {(landing.sampleEvents || []).map((event) => (
          <tr key={event.id} className="border-b border-border last:border-0">
            ...
            <td className="px-4 py-3 align-top">
              <StatusBadge status={event.readiness === 'ready' ? 'live' : event.readiness === 'blocked' ? 'error' : 'incomplete'} label={readinessLabel(event.readiness)} />
...
function LandingDetailEditor({
  ...
  return (
    <Card className="border-primary/30 p-4 shadow-sm">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="text-xs font-semibold uppercase text-muted-foreground">Полный редактор лендинга</div>
          <h2 className="mt-1 text-lg font-semibold">{detail?.landing.title || detail?.rule.title || 'Загрузка…'}</h2>
          <p className="mt-1 font-mono text-xs text-muted-foreground">{detail?.slug}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {isLoading ? <Badge variant="outline" className="gap-1"><Loader2 className="h-3 w-3 animate-spin" /> загрузка</Badge> : null}
          {isSaving ? <Badge variant="outline" className="gap-1"><Loader2 className="h-3 w-3 animate-spin" /> сохранение</Badge> : null}
      ...
      {detail ? (
        <>
          <div className="mb-4 grid gap-3 md:grid-cols-5">
            <MetricCard value={detail.metrics.effectiveEvents} label="В выборке" />
            <MetricCard value={detail.metrics.autoEvents} label="Авто" />
            <MetricCard value={detail.metrics.pinnedEvents} label="Закреплено" />
            <MetricCard value={detail.metrics.excludedEvents} label="Исключено" />
            <MetricCard value={detail.metrics.reviewEvents} label="На проверке" />
          </div>

          <div className="mb-4 grid gap-3 lg:grid-cols-[1fr_280px]">
            <div className="rounded-lg border border-border bg-secondary/20 p-3">
              <div className="mb-2 text-xs font-medium text-muted-foreground">Правило</div>
              <div className="flex flex-wrap gap-1.5">
                {(detail.rule.keywords || []).map((item) => <Badge key={`kw:${item}`} variant="outline">ключ: {item}</Badge>)}
                ...
                {(detail.rule.requiredTags || []).map((item) => <Badge key={`tag:${item}`} variant="secondary">тег: {item}</Badge>)}
                {(detail.rule.excludedTags || []).map((item) => <Badge key={`ex:${item}`} variant="outline">искл. тег: {item}</Badge>)}
                {(detail.rule.excludedKeywords || []).map((item) => <Badge key={`exkw:${item}`} variant="outline">искл.: {item}</Badge>)}
              </div>
            </div>
            <div className="rounded-lg border border-border bg-secondary/20 p-3">
              <div className="mb-2 text-xs font-medium text-muted-foreground">Публичное SEO</div>
          ...
          <div className="mb-4 flex flex-wrap items-center gap-3">
            <Input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder="Поиск событий в лендинге..." className="h-9 min-w-[260px] flex-1" />
        ...
        <InfoNote>Выберите лендинг или дождитесь загрузки деталей.</InfoNote>
      )}
    </Card>
  );
}
...
function LandingBlocksPreview({ blocks }: { blocks: NonNullable<AdminLandingDetail['blocks']> }) {
  ...
  return (
    <section className="mb-4 rounded-lg border border-border bg-card p-3">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="text-sm font-semibold">Контентные блоки</div>
          <div className="mt-1 text-xs text-muted-foreground">Публичная композиция: hero, ценностные блоки, сетка городов, FAQ и SEO-текст.</div>
        </div>
        <Badge variant="outline">{formatNumber(sorted.length)} блоков</Badge>
      </div>
      {sorted.length ? (
        <DataTableShell columns={['#', 'Тип', 'Заголовок', 'Вариант', 'Вкл.']}>
          {sorted.map((block, index) => (
            <tr key={block.id || `${block.type}:${index}`} className="border-b border-border last:border-0 hover:bg-secondary/40">
              ...
              <td className="px-4 py-3 text-xs text-muted-foreground">{block.isEnabled === false ? 'выкл' : 'вкл'}</td>
            </tr>
          ))}
        </DataTableShell>
      ) : (
        <InfoNote>Сохранённых блоков пока нет. Сервер отрисует запасные блоки из правила лендинга.</InfoNote>
      )}
    </section>
  );
}
      ...
      <DataTableShell columns={['Событие', 'Город / площадка', 'Дата', 'Цена', 'Включение']}>
        {events.map((event) => (
          <tr key={`${title}:${event.id}`} className="border-b border-border last:border-0">
            ...
            <td className="px-4 py-3 align-top">
              <div className="flex flex-wrap gap-1">
                {event.isAutoMatch ? <Badge variant="outline">авто</Badge> : null}
