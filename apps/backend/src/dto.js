      ...
      `
        ...
        raw_events as (
          select
            e.id,
            e.kind,
            e."sourceStatus" as source_status,
        ...
        grouped_events as (
          ...
          where price_from >= $1
            ...
            and (
              ...
              or kind = 'OPEN_DATE'
              or source_status = 'open_date'
              or source_code = 'TEPLOHOD'
            )
          group by
...
function landingRequiredSignalsSatisfied(rule, keywordFields, allowVenueTitleOverride = false) {
...
function collectFastLandingMatchReasons(event, rule, tags) {
  ...
  if (!reasons.length && rule.requiredAnyVenueKeywords?.length) {
    const venue = [event.venue, ...tags].filter(Boolean).join(' ').toLowerCase();
...
function landingKeywordTags(tags) {
  return (tags || []).filter((tag) => {
    const value = String(tag || '').trim();
    return value && !/^(Теплоход|Площадка):/i.test(value) && !DURATION_CATALOG_TAG_RE.test(value);
  });
}
...
export async function runLandingAudit(db, rootDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..', '..')) {
  ...
  for (const [landingSlug, externalIds] of Object.entries(expectations)) {
    ...
    for (const externalId of externalIds) {
      ...
      const openDate = Boolean(auditRow && isOpenDateCatalogRow(auditRow));
      const blockers = [];

      if (!auditRow) blockers.push('нет в БД');
      if (auditRow && futureSessions === 0 && !openDate) blockers.push('нет будущих сеансов');
