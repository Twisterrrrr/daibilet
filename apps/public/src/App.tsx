...
function StaticInfoPage({ kind }: { kind: 'blog' | 'privacy' | 'legal' | 'offer' }) {
  const meta = {
    blog: {
      title: 'Статьи',
      text: 'SEO-блог появится после запуска каталога: городские гиды, подборки по датам, маршруты, советы перед покупкой и перелинковка с событиями.',
    },
    privacy: {
      title: 'Политика конфиденциальности',
      text: 'Черновик раздела. До запуска здесь нужно закрепить состав персональных данных, срок хранения и порядок обработки заявок и статусов билетов.',
    },
    legal: {
      title: 'Правовая информация',
      text: 'Черновик раздела. Важно явно указать, что финансовый контур, чеки и платежи остаются на стороне билетных систем.',
    },
    offer: {
      title: 'Оферта для партнеров',
      text: 'Черновик раздела для поставщиков, площадок и будущего supplier-кабинета.',
    },
  }[kind];
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <main className="container-page py-16">
        ...
        <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">{meta.text}</p>
        <div className="mt-8 rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900">
          Это функциональная MVP-заглушка, чтобы навигация не вела в пустоту. Перед релизом раздел нужно заменить финальным текстом.
        </div>
      </main>
      <Footer />
    </div>
