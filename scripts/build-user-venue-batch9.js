...
const sourcePath = path.join(__dirname, 'data/venue-content-user-batch9-source.txt');
...
function main() {
  ...
  const venues = prod.items.filter((v) => INSTITUTION_KINDS.has(v.type));
  ...
  const payload = {
    ...
    source: 'user-batch9-institutions-without-description',
    venues: matched,
  };
