...
const sourcePath = path.join(__dirname, 'data/venue-content-user-batch10-source.txt');
const prodPath = path.join(__dirname, 'data/venues-without-description-prod.json');
const batch9Path = path.join(__dirname, 'data/venue-content-user-batch9.json');
const outPath = path.join(__dirname, 'data/venue-content-user-batch10.json');
...
function scoreMatch(entry, venue) {
  ...
  if (entryCity && venueCity) {
    const cityOk =
      entryCity === venueCity ||
      venueCity.includes(entryCity) ||
      entryCity.includes(venueCity) ||
      (entryCity === 'москва' && venueCity.includes('московск')) ||
      (entryCity === 'дубна' && venueCity.includes('дубна'));
    if (!cityOk) return -1;
  }
...
function main() {
  ...
  const doneIds = new Set(batch9.venues.map((v) => v.id));
  const venues = prod.items.filter((v) => INSTITUTION_KINDS.has(v.type) && !doneIds.has(v.id));
  ...
  for (const entry of entries) {
    ...
    for (const venue of venues) {
      if (usedIds.has(venue.id)) continue;
  ...
  const payload = {
    ...
    source: 'user-batch10-remaining-institutions',
    venues: matched,
  };

  fs.writeFileSync(outPath, `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
  console.log(`Matched: ${matched.length}/${entries.length} (pool: ${venues.length})`);
  if (unmatched.length) {
    console.log('Unmatched:');
    for (const item of unmatched) console.log(`  - ${item.title} (${item.city})`);
  }
  const unmatchedVenues = venues.filter((v) => !usedIds.has(v.id));
  if (unmatchedVenues.length) {
    console.log('Venues still without batch10:');
    for (const v of unmatchedVenues) console.log(`  - ${v.name} (${v.city})`);
  }
}
