...
Home (`/`) **не** ждёт GPS перед показом города: cookie/`localStorage` сразу, чтобы рельсы не мигали «все города». GPS по-прежнему может перебить stale storage после ответа.
