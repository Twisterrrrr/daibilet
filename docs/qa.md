...
В каталоге Дайбилет (repo seeds) отдельных festival-событий с этими именами нет. Блок хаба **не** вторая афиша: upcoming после пригородов (на tourist-хабах - после афиши), прошедшие открытой колонкой рядом с текущим/ближайшим (не `<details>`).
...
1. Прошедшие якоря сезона (Дягилев, KAMWA) - **LOCKED 2026-08-14 (обновлено):** не в сетке upcoming. Desktop: открытая колонка рядом с текущим/ближайшим; mobile: стек. Не свёрнутый `<details>`.
...
2. **ХМАО / Карелия:** ~~у ХМАО в `region-hubs` центр Ханты-Мансийск (есть ли saleable standalone?).~~ → **закрыто 2026-08-17:** Ханты-Мансийск - адмцентр (`region-hubs.centerCity`), карточка `/cities` с ≥1 событием (3 READY не прячем). **Карелия:** Петрозаводск по-прежнему не в `City`; **Сортавала** dual membership - страница/search сразу, сетка только при events ≥ 3 (порог снижен 2026-09-20 с > 5), до порога сессии и child на республике. Region hub Карелии на Петрозаводск не меняли.
...
| **Код** | Hard-limit в UI на 5 нет; лимит маршрута my-day остаётся общим (`DAY_ROUTE_MAX=20`). |
