...
3. Поиск в разделе **один** (`/places?q=`): музей и набережная в одном списке с тегом семейства. Каталоги `/venues` и `/locations` для обзора; их поле поиска тоже ведёт в смешанную выдачу. **Мой день:** один поиск в шапке подбора (места + события, тег семейства), не три поля. Mixed grid без запроса (`UX.LOC9`) - later.
