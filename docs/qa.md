...
3. Поиск в разделе **один** (`/places?q=`): музей и набережная в одном списке с тегом семейства. Каталоги `/venues` и `/locations` для обзора; их поле поиска тоже ведёт в смешанную выдачу. Mixed grid без запроса (`UX.LOC9`) - later.
4. `UX.LOC3` («Места и точки сбора») - superseded коротким лейблом «Места».
5. Потенциальные площадки (музей / театр / цирк / зал / филармония / ДК) **сразу в `/venues`** при create/seed. Нет билетов ≠ `/locations`. Дворцы/соборы как достопримечательности остаются локациями.
...
5. **Единый `/places` mixed grid без запроса:** ⏳ later; поиск раздела уже единый.
