...
### Наблюдения
- Каталог подборок визуально рвал афишу отдельной секцией и sticky «Подборки».
- Блог уезжал под площадки; FAQ висел отдельно внизу.

### Решения
- Подборки внутри непрерывной зоны `#affiche` (События → каталог сразу ниже). Sticky «Подборки» убран; `#directions`/`#more` → `#affiche`/`#faq`.
- Плитки подборок: full-bleed фото из `landing-images` (+ gradient fallback), без серой meta-полосы.
- Низ хаба: desktop 2 колонки FAQ | Из блога (`#faq`); mobile FAQ затем блог. Sticky: FAQ; «Из блога» только в mobile «Ещё» (alias → `#faq`).
...
### Наблюдения
- Owner: ритм заголовков/отступов с Lovable Perm mock; время посещения на карточках «Что посмотреть»; лайфхаки один голубой тон + CTA на одной линии; hover как у city cards на главной; фестивали должны быть 50/50 + прошедшие в 2 колонки (не узкая колонка справа).

### Решения
- Общий `CityHubSectionHeading` + `HUB_SECTION_PAD` (~80-100px) на секциях хаба.
- `visitMinutes` в cityInfo (optional); UI chip с часами только при данных. Пермь/канон заполнен; театры и гастро без выдуманного времени.
- Лайфхаки: `bg-sky-50` всем карточкам, CTA `mt-auto`, hover `-translate-y-1` + синяя тень как `CityCard` light.
- «События региона»: `md:grid-cols-2`; прошедшие `sm:grid-cols-2`.
...
### Наблюдения
- Owner: карточки лайфхаков слишком однотонные. Вернуть фон «Факта». Скролл: сценарии → пригороды → лайфхаки.

### Решения
- Лайфхаки: 6 muted pastel (`sky/amber/emerald/violet/rose/teal-50`) по индексу, читаемый текст, CTA без изменений.
- `CityHookFactCallout`: снова янтарный фон и полоса.
- Порядок: must-see → сценарии → пригороды → лайфхаки. Sticky-лейблы без изменений.
...
### Наблюдения
- Owner: «Главные места» / «Что посмотреть» на desktop должны быть одной горизонтальной лентой, не двумя рядами (карточки 2,4,6,8 под 1,3,5,7).

### Решения
- `CitySightsMustSeeList`: md+ карусель (≥4 места) - однорядный flex/snap как identity, ширина карточки `min(22rem, calc(50vw-3rem))`, стрелки prev/next в шапке без изменений.
- Mobile 80/20 peek и sparse-grid (<4) без изменений. Лайфхаки `beforeScenarios` не трогали.
- Live не выкатывали (owner не сказал «выкатывай»).
...
## 2026-08-14 - Lifehacks: all cards, no category tabs

### Наблюдения
- Owner: блок «Лайфхаки …: как сберечь бюджет» ок, но вывести все сразу без фильтров Пешком / Транспорт / Перелёт / Еда.

### Решения
- `CityLifehacksSection`: убран tablist; в карусели сразу все `pack.items` (порядок пака). Sticky `#lifehacks` без изменений. Live не выкатывали.
...
### Наблюдения
- Owner: внутри слитой правой карточки погода и «Когда ехать» поровну 50/50, чтобы сезон-пилюли аккуратно влезли (2x2 wrap). Внешний ряд факт ~40% / карточка ~60% без изменений. Сразу выкатить.
...
### Наблюдения
- Owner: не три отдельные карточки. Два внешних столбца: слева факт, справа (~60%) одна слитая карточка погода + «Когда ехать». Внутри карточки столбик погоды и сезоны у правого края допустимы.

### Решения
- Внешний ряд без изменений: `2fr` / `3fr` (факт | `CityWeatherWidget`). Сезоны не выносим соседней колонкой к факту.
- Внутри виджета: узкая колонка прогноза + сезоны через внутренний разделитель, без большого gap, чтобы не читалось как третья внешняя карточка.
- Мобилка: факт, затем слитая карточка. Live не выкатывали.
...
### Наблюдения
- Owner: карточка погоды+сезонов слишком высокая. Прогноз (сегодня / завтра / послезавтра) шёл широкой строкой, «Когда ехать» стояло под ним.

### Решения
- `CityWeatherWidget` на md+: две колонки в одной карточке - слева вертикальный прогноз, справа табы и совет «Когда ехать».
- Мобилка: по-прежнему столбик (погода, затем сезоны). Внешний ряд факт ~40% / погода ~60% без изменений.
- Live не выкатывали.
...
### Наблюдения
- Owner: лайфхаки карточками как «Чем уникальна Пермь», поставить перед «Готовые сценарии».

### Решения
- `CityLifehacksSection`: горизонтальная карусель (ширина карточки как identity), стрелки, табы сверху, H2 внутри секции `#lifehacks`.
- Рендер `beforeScenarios` в must-see блоке, перед пресетами дня.
- Sticky tab «Лайфхаки» → `#lifehacks`; «Советы» / `#practice` отдельно для travel+FAQ.
...
### Наблюдения
- Owner: в меню оставить «Зачем ехать»; H2 блока - «Что посмотреть в Перми»; не плодить сценарии из «Чем уникальна»; поменять местами факт и погоду; выкатывай.

### Решения
- Tab/jump: «Зачем ехать». H2: `Что посмотреть ${cityIn}`.
- Identity: только scroll к must-see/suburbs, без focus-баннера «N точек / Сбросить».
- Desktop: факт слева ~40%, погода справа ~60% (`2fr`/`3fr`).
...
### Наблюдения
- Owner: на макете погода/факт примерно 60/40; у нас погода была узкой (~42%).

### Решения
- Desktop: `md:grid-cols-[minmax(0,3fr)_minmax(0,2fr)]` - погода слева шире, факт справа.
...
### Наблюдения
- Owner: «Лайфхаки по Пермь» - нужно «по Перми». Проверить все города.

### Решения
- Предлог **по** + дательный: Перми, Москве, Санкт-Петербургу, Калининграду, Нижнему Новгороду.
- `cityToDative` / `poCityDative` в `city-declension.ts`. H2: `Лайфхаки ${poCityDative(city.name)}: как сберечь бюджет`.
...
### Наблюдения
- Owner (Кунгур): тёмный блок «Перенесем эти N точек в интерактивный планировщик» виден только полоской у нижнего края карточки.

### Решения
- Тултип `CollectRouteCtaHint` открывается **над** кнопкой, не под ней.
- С `DayTripCanonCard` снят `overflow-hidden`; скругление cover остаётся на самом фото.
...
### Наблюдения
- Owner: причесать стили заголовков, шрифтов, фонов, бордеров, оформления хаба. Макет: identity с подзаголовком и бейджами, must-see с подсказкой «В маршрут», блог «Из блога о Перми».

### Решения
- Фон tourist hub: `bg-slate-50`, карточки белые с мягкой тенью и `border-slate-200/90`.
- Identity: lead, navy pill на фото, H2 без вопросительного.
- Must-see: IA карусели и H2 «Зачем ехать» сохранены; карточки, чипы, счётчик маршрута, подсказка.
- Блог: «Из блога о {предложный}», lead, «Все материалы», тег + время чтения.

### Проблемы
- Нет. Live не трогаем, пока owner не скажет выкатить.
...
### Наблюдения
- Owner: поменять факт и погоду местами, вернуть цветной фон, убрать точки под каруселью уникальности (стрелки справа сверху уже есть), такие же стрелки у «Зачем ехать», в пригородах фото слева и текст справа.

### Решения
- Desktop: факт слева, погода справа. Мобилка: погода сверху.
- Погода: градиент sky→indigo на прогнозе. Факт: янтарный фон и полоса.
- Точки-пагинация identity сняты. Стрелки карусели «Зачем ехать» в шапке секции, как у уникальности.
- Хаб-карточка пригорода: cover слева, описание справа (`sm+`).
...
## 2026-08-14 - City hub: tourist affiche and Perm IA

### Наблюдения
- Owner-аудит Перми: страница смешивает гид и афишу, стендап в топе бьёт УТП, Хохловка раздувает скролл до билетов, прошедшие фестивали занимают сетку.

### Решения
- Tourist-хабы (Пермь, Мск, СПб, КГД, НН): дефолтный чип «Экскурсии» если есть в выдаче; ранг экскурсии/река → театр → музеи → концерты → стендап. Таб «Музеи» скрыт при <3 карточках. 3+ стендапа схлопываются в виджет дат.
- SSR slice 48 на тех же городах сортируется тем же скором, чтобы экскурсии не вытеснялись стендапом до клиента.
- Афиша сразу после сценариев; пригороды, фестивали и блог ниже. Фильтры афиши sticky.
- Nested POI пригорода: 3 пункта + «Ещё N». Прошедшие фестивали сначала были в details; с 2026-08-14 owner: открытая колонка рядом с текущим (см. UX.HUB-FEST-PAST-COL).
- Екатеринбург не трогали: там стендап - curated primary.

### Проблемы
- Нет двухколоночного desktop и верхних табов «Афиша / О городе» - отдельный IA-шаг. Email-тизер музеев не делали (таб просто скрыт). Live после «выкатывай».
...
### Наблюдения
- Owner дал пакеты ещё на 4 города: Москва (только лайфхаки), СПб / Калининград / Нижний (погода+сезоны и лайфхаки). Табы сезонов на мобилке ломали ширину.

### Решения
- Лайфхаки в `city-hub-lifehacks.ts`: Москва, СПб, Калининград, Нижний. Пустые вкладки не рисуем (у СПб нет «Перелёт»). Fly CTA остаётся афишей хаба, не агрегаторами.
- Калининград: обязательная ссылка экосбора `https://www.park-kosa.ru/`.
- Погода/сезоны: СПб, Калининград, Нижний. Москва без виджета, пока нет сезонного текста.
...
### Наблюдения
- Owner: у «Чем уникальна Пермь» лишний нижний скролл при карусели и стрелках. Якорь «Зачем ехать» в шапке должен вести на этот блок.

### Решения
- Скроллбар карусели скрыт (`scrollbar-width: none` + webkit/ms). Стрелки на всех ширинах.
- `#sights` на карусели, если она есть. Без карусели якорь остаётся на секции мест.
...
## 2026-08-14 - Perm hub: fact + weather cards aligned

### Наблюдения
- Owner: факт и погода визуально спорят. Разная высота, оранжевый vs градиент, «данные недоступны», конфликт «Конец лета (август)» при выбранной «Зиме».

### Решения
- Мобилка: колонка, сначала сезоны/погода, под ним факт. Десктоп `md+`: ряд, `align-items: stretch`, одинаковая высота.
- Общий chrome карточек: белый, `rounded-2xl`, тихий kicker slate, без янтарной полосы и без сине-фиолетового градиента.
- Прогноз рисуем только если API ответил. Падение Open-Meteo не показывает ошибку - остаются табы сезонов.
- Один текст под табами: в августе на «Лето» - copy «конца лета» и подпись «Сейчас». Клик «Зима» убирает августовский заголовок.

### Проблемы
- Live после web deploy.

---

## 2026-08-14 - Perm weather: empty widget was a routing bug

### Наблюдения
- Owner: на хабе Перми «Сейчас данные недоступны», хотя Гисметео и Яндекс.Погода живые.
- Live `GET /api/public/weather/perm` → backend `{ error: not_found, path }` 404. nginx `/api/public/*` идёт на каталог `:4000`, Next-роут погоды туда не доезжал. Open-Meteo с этой машины отвечает 200 (~18 °C, код 80).

### Решения
- Не парсим Гисметео/Яндекс.Погоду: это закрытые API без нашего ключа, HTML-информеры ломают UI.
- Тот же прогноз Open-Meteo (ECMWF/DWD), но путь `GET /api/day-route/weather/:slug` - префикс уже проксируется на Next.
- Если Next-роут недоступен, виджет сам дергает `api.open-meteo.com` из браузера. Таймаут 8с, 2 попытки, без кэша пустого ответа.

### Проблемы
- На live виджет оживёт после web deploy. Owner не писал «выкатывай» в этом сообщении.
...
### Наблюдения
- Owner: на хабе города нужен блок «Лайфхаки по {city}: как сберечь бюджет». Первый контент - Пермь. На мобилке короткие карточки + CTA, без длинных абзацев. Четыре таба, не три: Пешком / Транспорт / Перелёт / Еда.

### Решения
- Данные в `apps/web/src/lib/city-hub-lifehacks.ts` (getter по `normalizeCityHubSlug`), не в `cityInfo`. Другие города - null, секция «Советы» как раньше.
- `#practice` сохраняется для sticky tabs; H2 и лейбл таба для Перми - «Лайфхаки». Внутренний якорь `#lifehacks`.
- Длинный `CityTravelSection` для Перми скрыт (`skipTravel`): карточки закрывают ту же тему. FAQ и seasonal tip остаются ниже.
- CTA: Яндекс/2ГИС (внешние), афиша хаба (`scrollToSection('affiche')`), гастро-фокус через тот же `applyPlaceFocus`, что у карусели уникальности. Не авиаагрегаторы, не wide catalog.
...
### Наблюдения
- Owner вставил lat/lng для 184 строк SPB mustSee. Строки 46 и 162 на картинке пустые (дворы-колодцы и булочная Вольчека) - не трогали.
- В mustSee между дацаном и Анненкирхе есть соборная мечеть (не из таблицы 184) - координаты мечети оставили.

### Решения
- 182 точки: `latitude`/`longitude`/`address` в web+public `cityInfo.ts` и `SAINT_PETERSBURG_COORDS` (хаб и My Day совпадают). Адрес редактора важнее старого (Ахматова - Фонтанка 34, Планетарий №1 - Обводный 74Ц).
- Lock `scripts/data/spb-owner-editor-coords.json` подключен к Yandex-audit как OWNER_OVERRIDES, чтобы геокод не затер пины wiki-центроидами.
- Live не выкатывали (канон batch).

### Проблемы
- Нет. Несматченных имен не было.
...
### Наблюдения
- Owner: памятники 5 городов (СПб/Мск/НН/КГД/Пермь) не видны в хабах и во вкладке «Главные» My Day.
- Пак `catalog-monuments-2026-08-13.json` (116) 2026-08-13 сознательно не лили в `cityInfo.mustSee` (только каталог `/locations`, MSK DB 108 insert / 8 update). Хаб и My Day читают только curated mustSee.
- В mustSee уже были 8 slug из пака; остальные 108 в хаб не попадали. Отдельного фильтра `monument` нет: без override kind MONUMENT идёт в `main`.

### Решения
- Merge-модуль `city-monuments-must-see.ts` (web+public): недостающие точки пака дописываются в `CITY_INFO.mustSee` при загрузке, без правки огромных секций cityInfo.
- Иконические → `mustSeeFilter: 'main'`; жанровые (хомлины, Чижик, Никулин, сырок и т.п.) → `creative` («Необычное»). Дубликаты slug не трогаем (Кант/зубры остаются park).
...
## 2026-08-14 - My Day: editable dwell time

### Наблюдения
- Owner: пользователь хочет сам задавать длительность посещения точки.

### Решения
- Поле `dwellMinutes` в localStorage stop; chip «~N мин на месте» кликабелен (пресеты + своё + Авто).
- Hour-plan и оценка дня берут override, иначе soft default по типу.

### Проблемы
- В share `?items=` / short URL dwell пока не кодируется (остаётся локально у автора). Live после «выкатывай».
...
### Наблюдения
- Owner: точно выделить галереи, достопримечательности, памятники, храмы, гастроточки.

### Решения
- Чипы: Галереи (`art_space`), Достопримечательности (`attraction`), Памятники, Храмы (`temple`), Гастроточки (`gastro`).
- Музеи без art_space; бары без gastro; парки без attraction.
- Public kind `temple` из attraction/outdoor_location по названию (собор/церковь/храм/монастырь/…).

### Проблемы
- Отдельного Prisma enum TEMPLE нет - только public resolve; CMS kind остаётся ATTRACTION/OUTDOOR_LOCATION.

---

## 2026-08-13 - /places: понятные категории, Фильтры, сорт у счётчика

### Наблюдения
- Owner: бейджи Площадка/Локация лишние; чипы kind слишком технические; scope «Только площадки/локации» путает; сорт далеко от «N мест».

### Решения
- Бейджи family с карточек убраны.
- Чипы: Музеи / Театры / Концертные залы / Бары и рестораны / Парки и открытые места (группы kind; API `type=a,b,c`).
- Scope в меню «Фильтры» (ползунки): Показывать все / Только с событиями. Без жаргона площадка/локация.
- Сорт на одной линии со счётчиком мест, справа.
- Eyebrow: «N мест • M городов».

### Проблемы
- «Доступно сегодня» в фильтры не выносили - нет стабильного поля на каталоге мест; при необходимости отдельный хвост.
...
### Наблюдения
- Owner: ряд «Показывать все / Только площадки / …» - выводить через `•`; «Собрать день» в этом ряду лишнее.

### Решения
- Scope-тогглы: `Показывать все • Только площадки • Только локации • Только с событиями`.
- Ссылку «Собрать день» убрали из chrome `/places` (маршрут остаётся через карточки / хедер / my-day).
...
## 2026-08-13 - /places: Башкортостан падеж, город на карточке, серый «В маршрут»

### Наблюдения
- Owner на `/places` при destination «Республика Башкортостан»: H1 «…Республики Башкортостана» (лишняя «а»); на карточке город только из shortDescription; фильтр «Город» показывает регион; «В маршрут» синий, на хабе города - серый.

### Решения
- `CITY_FORMS`: Башкортостан / Республика Башкортостан без склонения второй части → «Республики Башкортостан».
- `venue-place-city.ts`: при region-like `City.title` берём населённый пункт из `citySlug` (Стерлитамак, Нефтекамск, Октябрьский…). Карточки показывают его в meta/адресе.
- `/places` dropdown в регионе: опции населённых пунктов (+ «вся республика»), не национальный список регионов.
- `AddToDayRouteButton` на Institution/Location card: default `light` (серый), как на city hub.

### Проблемы
- В БД у части City title = регион при slug города - UI-обход; канон title населённого пункта - отдельный data-fix.
...
### Наблюдения
- Owner: строка поиска на `/events` прилипает к кнопке «Найти»; серый `#F5F5F7` без воздуха. Нужен бордер и белый фон.

### Решения
- `CatalogToolbar` form: `bg-white` + `border-slate-200`, gap `0.5rem` между полем и «Найти»; input прозрачный внутри оболочки.
- Idle «Фильтры»: белый + бордер вместо серой заливки.
...
### Наблюдения
- Owner: кнопка «Афиша» на карточке места бессмысленна - клик по карточке и так ведёт на PDP. Нужен «В маршрут» на этом месте и сердечко в правом верхнем углу, как у событий.

### Решения
- InstitutionCard / LocationCard: overlay - `PlaceFavoriteButton` (отдельный LS `daibilet:place-favorites`, панель избранного показывает места).
- Низ карточки: compact «В маршрут» (не внутри Link). «Афиша»-декор убран.
...
## 2026-08-13 - /places: фиксированный H1, длинный description, одна синяя кнопка

### Наблюдения
- Owner: вернуть H1 `Музеи, театры, локации, достопримечательности {город}`. Description короткий. Два ряда чипов с двумя синими кнопками («Показывать все» + «Все типы») грузят шапку; раньше подача была легче.

### Решения
- H1/title фиксированные, город в родительном. Shuffle kind от slug убран.
- Description длиннее: площадки, парки, набережные, билеты и маршрут на день.
- Чипы kind: одна синяя (`Все места` или выбранный тип). Scope - текстовые ссылки в строке с «Собрать день», без второй синей кнопки.
...
## 2026-08-13 - /places: сорт «По событиям» игнорировал STOP

### Наблюдения
- Owner: сортировка на `/places` работает хреново. «По событиям» сравнивал только own `events`. Карточка локации показывает `stopEventCount` («N в маршрутах»), поэтому Эрмитаж/Адмиралтейство уезжали вниз относительно бара с 1 событием. Shell ещё обнулял `events` до сортировки - получалось как A-Я.

### Решения
- `sort=events` = `max(events, stopEventCount)`, как бейдж на карточке.
- Shell (`counts=0`) не используется для events/mixed. Сортировка до обнуления page payload.
- Селект пишет `?sort=` в URL.

### Проблемы
- Алгоритм в `daibilet-api`. Deploy MSK web сам API не рестартит. На live - web deploy + restart API, либо «выкатывай».
...
- Hero событий/локаций/площадок на мобилке широкий и низкий (16:10 / min-h ~42vh) - кроп выглядит плохо.

### Решения
- Порядок колонки: маршруты причала (если есть) → О локации → Как добраться → Рядом/экскурсии → слоты → children.
- Mobile hero: `aspect-[3/4]`, текст внизу кадра; с `sm+` высота как раньше.
...
## 2026-08-13 - /places: H1 из четырёх типов + сорт «По событиям»

### Наблюдения
- H1 «Музеи, театры, площадки, локации {города}» один на все города. Owner: можно четыре типа + родительный; рандом на запрос бить SEO нельзя.
- Сорт `events` подписан «По афише» - непонятно, что это число событий.

### Решения
- H1/title: четыре kind из пула, shuffle от slug города (один URL - один заголовок). Национальный `/places` без города остаётся дефолтным набором. Description широкий, без копирования четырёх слов.
- Подпись сортировки: «По событиям» (режим API тот же - `sort=events`).
...
## 2026-08-13 - /places: карточки локаций как площадки + тоггл scope

### Наблюдения
- Каталог один (`/places`), но карточка достопримечательности жила в другом chrome: тип на фото вместо uppercase meta, описание выше адреса, CTA «В маршрут» слева, без overlay share.
- Owner: выровнять под venues и вернуть тоггл Показывать все / Только площадки / Только локации / Только с событиями.

### Решения
- `LocationCard` - тот же каркас, что `InstitutionCard`: aspect-video, overlay «в маршрут», бейдж «Локация» на mixed grid, тип·город uppercase, адрес, описание, синяя «Афиша» справа.
- Тоггл пишет `family=institution|location` или `hasEvents=1`. Чипы типов отдельно («Все типы»). `hasEvents` - thin/noindex; API не ходит в shell (иначе events=0 и список пустой). В фильтр входят own events и STOP.
...
## 2026-08-13 - /places: чипы фильтров не обрезать

### Наблюдения
- Owner: ряд типов на `/places` обрезает «Достопримечательности» справа. Скроллбар скрыт, на десктопе ряд выглядит обрезанным.

### Решения
- Чипы `flex-wrap`, без `nowrap` + hidden overflow. Places / venues / locations один паттерн.
...
## 2026-08-13 - /places: не показывать два family-каталога

### Наблюдения
- Owner: `/places?city=perm&family=institution` и `family=location` - зачем это отображать.

### Решения
- Для человека один каталог `/places?city=perm`. Family (площадка vs локация) остаётся внутренним: kind, PDP `/venues` vs `/locations`, 301 со старых листингов.
- С хаба сняты ссылки «Площадки с афишей» / «Локации…»; из футера тоже. Чипы типов (музей, парк) остаются.
...
## 2026-08-13 - Mobile: скролл застревает на событиях и статьях

### Наблюдения
- Owner: на мобилке нельзя листать страницу вниз, если палец попадает на карточку события/статьи (данные API в горизонтальных лентах).

### Решения
- Причина: `touch-action: pan-x` на `.horizontal-snap-row` / `ScrollRail`. Высокая карточка занимает почти весь экран, браузер запрещает вертикальный жест.
- Канон: `touch-action: pan-x pan-y` на `.horizontal-snap-row`. Снят Tailwind `touch-pan-x` (он перебивал бы правило). Горизонтальный свайп ленты остаётся.
...
## 2026-08-13 - My Day: поблизости это события, не экскурсии

### Наблюдения
- Owner: «· 2» было про число офферов рядом, не про точки маршрута. Вопрос - все это или есть ещё. Писать «экскурсии» не нужно: это события.

### Решения
- Под заголовком «События поблизости»: «Которые задевают точки дня». Счётчик только если список обрезан: `показаны 3 из N` + «Показать ещё». Два карточки без кнопки = больше нет.
...
## 2026-08-13 - My Day: «Сменить город» как синяя ссылка

### Наблюдения
- Owner: «Сменить город» в подзаголовке серое, «Страница Перми» синяя. Нужен тот же вид.

### Решения
- `renderCityChangeControl`: `text-primary-600 hover:text-primary-700` + тот же `DAY_TEXT_LINK`, что у хаб-ссылки.
...
## 2026-08-13 - My Day: превью SPB / KGD / MSK / NN / Пермь

### Наблюдения
- Owner: не все превью в `/my-day` (карточки «Главные места» и обложки пригородов).
- Аудит mustSee: MSK 93 точки без editorial-карты (Кремль/Красная площадь/МГУ в том числе), SPB 2 (обрезанный slug Дома ученых, особняк Мясникова), KGD биржа без slug, NN 4 suburb hub без обложки. Пермь mustSee уже был закрыт.
- Lookup был exact-match: хвост `-` и чужой hub-slug (Павловск) давали пустой thumb, хотя файл на диске был.

### Решения
- `lookupEditorialPlaceImage`: trim trailing `-`; alias Павловск → уже сгенерированный cover.
- cityInfo: починен slug Дома ученых; Кёнигсбергская биржа получила `locationSlug` (фото биржи = музей ИЗО в том же здании).
- Новые GenerateImage covers + ключи в `city-place-images.ts`: MSK mustSee/suburb hubs, SPB Мясников/Гатчина, NN Городец/Семёнов/Дивеево/Макарьев.
- MustSee пяти городов: 0 дыр по карте+файлу. Вложенные POI пригородов без slug по-прежнему без своих thumb (карточка пригорода берёт обложку хаба).

### Проблемы
- Нет. Live после Deploy MSK web / «выкатывай». Вложенные точки пригородов без slug - отдельный хвост, не mustSee picker.
...
### Наблюдения
- Owner (скрин СПб): воздух над H1 в night-hero меньше, чем под кнопками Афиша / Подборки. Давно просил сделать верхний отступ таким же, как нижний.

### Решения
- `CITY_NIGHT_HERO.content`: равные `py-10` / `sm:py-12` и `justify-center` на всех брейкпоинтах. Снят `md:justify-end` + асимметрия `pt-16 pb-10` / `md:pt-12 md:pb-10`.
- Секция: `md:min-h-[360px]` без фиксированного `h-[360px]`, чтобы длинный лид+CTA не обрезались `overflow-hidden`. Navy/fade/gutter 16:9 без изменений.
...
## 2026-08-13 - Мой день: группы кнопок + счётчик экскурсий

### Наблюдения
- Owner: Оптимизировать / По часам / Удалить должны жить одной строкой и переноситься целиком; PDF и Сохранить - второй такой же блок.
- «События поблизости · 2» читалось как «2 точки». На маршруте 9 точек; на карточке «1 из 9».

### Решения
- Тулбар: три nowrap-кластера (режим / действия / экспорт) внутри `flex-wrap`. Кнопки внутри кластера не разъезжаются.
- Подпись: `2 экскурсии` (не голое «· 2»). Превью 3 карточки; при двух уникальных продуктах ничего не обрезаем. Дубли сеансов TC схлопываются в одну карточку.
...
## 2026-08-13 - Мой день: повторный клик убирает место

### Наблюдения
- Owner: площадки и локации логично не только добавлять в маршрут, но и снимать тем же кликом. И в Мой день, и в каталогах.

### Решения
- Каталожные кнопки (`compact` / `iconOnly`) больше не add-only: второй клик снимает точку, как на PDP.
- Подбор в Мой день (главные места, поиск, пригород пачкой): повторный выбор убирает, а не тост «Уже в маршруте».
...
## 2026-08-13 - Афиша `/events`: фильтры, комбо, шапка

### Наблюдения
- Owner: три слоя фильтров дублируют город/категорию/дату; поиск сливается с чипами; «Комбо 1…6» с одной обложкой; мелкий вид и «50 на стр.»; шапка не компактится при скролле.

### Решения
- Чипы «Активно» только для q / landing / диапазона дат / цены / возраста. Город, категория, день, «Бесплатно», «С детьми» остаются подсвеченными в своих лентах. Сброс не трогает город шапки.
- Поиск: контрастная плашка, на фокусе популярные запросы (Выставки, Концерты на выходных) плюс фасеты.
- Комбо на одной площадке схлопываются в одну карточку; обложка - фото venue; описание из list DTO под названием. Зачёркнутую цену не выдумываем (нет oldPrice).
- Вид grid/list крупнее; селект размера страницы снят; CTA пагинации «Показать ещё N».
- Шапка: скролл вниз прячет desktop-меню и снижает высоту (`--site-header-height`); скролл вверх возвращает.
...
## 2026-08-13 - My Day: без блока «График дня»

### Наблюдения
- Owner: отдельный Gantt «График дня» лишний. Достаточно времени на карточках точек.

### Решения
- Снят `MyDayHourGantt`. План по часам по-прежнему пишет интервалы на стопах (`softTimeLabel`). Баннер overflow/обед оставлен.
...
### Наблюдения
- Owner: после «Сохранить сценарий» снимок уходил только во вкладку «Готовые сценарии» в подборе. У Lovable чипы «Мои сценарии дня» стоят в левой ленте маршрута, под «Открыть подбор».

### Решения
- После сохранения чипы сразу в колонке маршрута (`MyDaySavedScenariosRail`), с фильтром по городу.
- В подборе список остаётся. Клик по чипу закрывает подбор и грузит снимок.
...
## 2026-08-13 - Каталог: пак памятников СПб/Мск/НН/КГД/Пермь

### Наблюдения
- Owner прислал списки памятников (базовые + доп. точки) и попросил добавить в каталог. Никулин в Москве в списке дважды - одна карточка.

### Решения
- Пак `scripts/data/catalog-monuments-2026-08-13.json` (116 локаций): СПб 35, Москва 35, Нижний 16, Калининград 15 (семья хомлинов), Пермь 15.
- Kind `MONUMENT` → `/locations`. Исключения: Триумфальная арка, «Счастье не за горами», бронекатер «у тюленя» → `ATTRACTION`.
...
## 2026-08-13 - /places: сразу смешанная лента, не два каталога

### Наблюдения
- Owner: https://daibilet.ru/places - два входа «С афишей» / «Достопримечательности» смотрятся странно. Нужна одна выдача.
- UX.LOC9 держали later; поиск уже был mixed, хаб без `q` оставался дверцами.

### Решения
- `/places` грузит `family=all` сразу. Чипы Все / С афишей / Достопримечательности фильтруют ту же сетку. Карточки по-прежнему `/venues` и `/locations`.
- Без запроса сорт `mixed`: площадка и локация чередуются, чтобы страница 1 не была только афишей.
- Узкие каталоги остаются ссылками внизу, не главной IA.
...
## 2026-08-13 - My Day picker: Выбор Дайбилет сеткой, не каруселью

### Наблюдения
- Owner: на десктопе горизонтальный скролл «Выбор Дайбилет» не нужен, карточки можно показать сеткой.

### Решения
- Grid 1→2 колонки по container query (`32rem`). Без `overflow-x` / snap. Мобилка и узкий sheet - столбик.
...
## 2026-08-13 - My Day picker: 2 колонки для главных мест

### Наблюдения
- Owner: в «Подбор точек → Главные места» широкие карточки логично бить на 2 колонки, если текст ещё влезает.
- В августе уже откатывали `sm:grid-cols-2`: viewport-breakpoint сжимал thumb+текст в узком sheet.

### Решения
- Container query на панели контента: 2 колонки с `min-width: 46rem` (~736px текста). Узкий sheet / мобилка остаются одной колонкой.
- Дефолт ширины picker 1120 (`picker-width.v2`), thumb чуть компактнее (`sm:h-20 w-24`), `line-clamp-3` + `title` с полным hook.
...
## 2026-08-13 - My Day: курсор бесится на ссылках города

### Наблюдения
- Owner: «Страница Санкт-Петербурга» / «Сменить город», затем «185 точек · 11 пригородов», затем любая ссылка на /my-day. Курсор pointer/default по кругу.
- Две причины. 1) Глобальный `button { min-height: 44px }` раздувает текстовые и icon-кнопки: невидимый hit-box наезжает на соседний текст (счётчик точек, хаб, заголовки стопов). 2) `hover:underline` + `items-baseline` / `items-center` в шапке меняет высоту строки, шапка перецентрируется, hover срывается.

### Решения
- В `@layer base`: `[data-day-list-root] button` (и topbar/picker/shell) `min-height: 0`. Явный `min-h-*` Tailwind по-прежнему сильнее.
- Текстовые ссылки/кнопки: класс `DAY_TEXT_LINK` (черта через `::after`, без сдвига line-box) + `inline-btn min-h-0` на button.
- Шапка: `lg:items-start`, счётчик не на `items-baseline`.
...
## 2026-08-13 - Live: Deploy MSK web `31697785067`

### Наблюдения
- Owner: «выкатывай». На live ещё стоял `31694312986` / `21d3c64` BUILD_ID=`DNWhE8Cu-cNOGBAQRWMDI`. В ветке уже были стопы (Адмиралтейство≠причал, Эрмитаж≠балет, честный вход) + аудит бесплатного входа.

### Решения
- Deploy MSK web `31697785067` success, ~3m29s. SHA `d8268c8fef9a0cebcb71a720ee0faa37a4762936`. BUILD_ID=`wOdU6cU5y7ITl_bsyiqqa`.
- Smoke: `/` `/my-day` `/places` → 200; новый BUILD_ID в HTML home.

### Проблемы
- Нет. Старый маршрут в localStorage гостя может держать причал/балет, пока не пересоберёт пресет.
...
### Проблемы
- Летний сад / Новая Голландия тоже без чипа: лучше молчать, чем врать про Петергоф. Live `31697785067` BUILD_ID=`wOdU6cU5y7ITl_bsyiqqa`.
...
### Проблемы
- Старый маршрут в localStorage может держать причал/балет, пока гость не пересоберёт пресет. Live `31697785067` BUILD_ID=`wOdU6cU5y7ITl_bsyiqqa`.
