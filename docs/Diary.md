...
## 2026-07-19 — ИИ-журналисты блога: персоны и стилевые гайды

### Наблюдения

- Нужны несколько устойчивых голосов колонок, а не единый «нейро-SEO» тон.
- У `Article` / `BlogPost` нет поля author — byline пока только через docs/frontmatter.
- Текущий каталог и уже опубликованные статьи сильнее завязаны на МСК/СПб культуру, реки, семью, театр/выставки, чем на Алтай/Байкал/гастро-регионы.

### Решения

- Созданы 5 вымышленных авторов + колонок в `docs/ai-journalists/` + `personas.json`.
- Style guides опираются на публичные URL-образцы прототипов; byline не копирует имена Птушкина/Парфенова и т.д. (`style_prototypes` + дисклеймер).
- Пилот рекомендовать: **Аглая Фонтанская** или **Мила Орлова** — лучший fit с текущей афишей.
- Статью не писать, пока пользователь не даст отдельный промпт темы.

### Проблемы

- Fair-use: в гайдах только короткие цитаты/маркеры; полные чужие тексты не копировать в контент блога.
- Расширение Prisma `authorId` отложено до появления реальных колоночных публикаций.
