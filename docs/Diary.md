...
- После PurchaseProjection и STUB/YooKassa smoke поставщик видит внутренние покупки, но таблица заказов все еще читалась как бухгалтерская выгрузка.
- Для первых музеев и арт-площадок важнее рабочий ответ: что уже оплачено и надо обслужить, что зависло в оплате, где отмена/возврат/ошибка.

### Решения

- В `apps/supplier` раздел "Заказы" получил фильтры статуса (`Все`, `Резерв`, `Ждут оплату`, `Оплачены`, `Выданы`, `Отменены`) поверх backend status query.
- Добавлена "Очередь обработки": к выдаче, ожидание оплаты, проблемные покупки. В карточках нет внутренних `CheckoutOrder.id`, только public code / короткий fallback.
- KPI заказов теперь показывают "К выплате" с оборотом в hint, чтобы поставщик сразу видел сумму после комиссии.
- Таблица сохранена как плотный режим сверки, но строки стали более человеческими: покупка, контакт покупателя, статус, комиссия, дата оплаты.
...
- В legacy `SPBBOATS/packages/frontend-supplier-v2` была удачная легкая оболочка ЛК: спокойный sidebar, компактный topbar, плотные таблицы, page primitives и разделение финансов/настроек.
- Текущий `apps/supplier` уже имеет живые read API, поэтому правильнее не портировать legacy целиком, а взять UX-паттерн и оставить наши finance DTO / PurchaseProjection.

### Решения

- Обновлен `apps/supplier`: grouped nav (`Работа`, `Деньги`, `Настройки`), Daibilet ticket-mark, более тихая рабочая визуальная система.
- Добавлены рабочие разделы: `Готовность`, `Документы`, `Команда`, `Интеграции`.
- Из основных таблиц поставщика убраны лишние технические подписи; интерфейс показывает бизнес-поля, статусы, цены, готовность и следующий смысловой шаг.
- Все новые экраны read-first и не используют моки: если backend не отдает сущность, показывается пустое состояние.
...
- Примечание: локально Node `v24.14.0`, проект ожидает `>=22.13.0 <23`, поэтому проверка запускалась с временным `engine-strict=false`.
