...
### Проблемы
- Локальный PostgreSQL и MCP-query недоступны, поэтому наличие строк подтверждено их публичными 200-карточками; финальный smoke выполняется после MSK deploy.
