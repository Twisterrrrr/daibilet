...
| # | Задача | Приоритет | Статус |
|---|--------|-----------|--------|
| UX.BLOG-DECOUPLE-CITY | `/blog`: отвязать ленту от header CityPicker; фильтр материалов сразу после hero; при активном фильтре результаты над «материалом недели» | Высокий | ✅ commit pending |
...
| BLOG.HIDE-AUG5-EXCEPT-BARS | Скрыть все статьи от 2026-08-05 кроме «Барный Петербург»; постепенная публикация | Критический | 🔄 MD ×5 DRAFT; публичный `spb-barnyy-peterburg-ryumochnye-spikizi`; нужен MSK upsert |
...
| MS.TIER5 | Phase 2 optional: `dayRoutePresets` / multi-day для MSK/SPB после стабильного широкого каталога | Низкий | ✅ СПб: 6 пеших кластеров с гастро-остановками, всегда видимый companion-гид и CTA планировщика только для маршрута с ≥3 разрешимыми точками |
