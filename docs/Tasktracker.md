...
| UX.BUY-10 | Ticket page `/checkout/ticket/{publicCode}` (HTML ticket + print + QR) | Критический | ✅ `ca8332f4` / follow-up order≠ticket |
| UX.BUY-10b | «Код заказа» once + «Номер билета» (пока = publicCode + caption); QR = ссылка на страницу; qa LOCKED draft order≠ticket | Критический | 🔄 deploy pending |
