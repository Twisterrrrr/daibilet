...
| # | Задача | Приоритет | Статус |
|---|--------|-----------|--------|
| UX.MS1 | «Главные места»: full blurb via ExpandableBlurb (`line-clamp-6` mobile / none desktop); без mid-word clip | Высокий | 🔄 deploy |
...
| UX.MS2c | Hub must-see mobile: peek ~80% + ExpandableBlurb; presets blog link new line + breathing room | Высокий | 🔄 deploy |
