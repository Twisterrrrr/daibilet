...
- Порядок secondary-блоков (после списка/карты): **Главные места → События поблизости → Добавить своё место** → ниже каталог/Hot Picks. Matches не уезжают вниз при наличии точек.
- Блок **«События поблизости»**: карточки с score покрытия (`3 из 3 точек`, `2 из 3`); без ряда «Места экскурсии не в маршруте».
- Пустые состояния:
