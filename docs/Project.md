...
2. **`significantSuburbs` - блок «Значимые пригороды {City_Род}»** на city hub (отдельно от mustSee). Для СПб: Петергоф, Царское Село, Кронштадт (и аналоги для других городов при необходимости). Пригороды формально в агломерации, но продуктово - отдельный day-trip слой.
