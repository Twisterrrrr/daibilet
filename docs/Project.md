...
- **Подборки city ЧПУ (2026-08-11, план):** owner хочет `/podborki/{city}` вместо soft `?city=` на хабе. **Не внедрено** - коллизия с `[intent]`, разъезд slug (`moskva` vs `moscow`). Канон плана: [seo-podborki-chpu-plan.md](./seo-podborki-chpu-plan.md); трек отдельно от My Day routes и category listing texts. Код/перелинковка только после ответов в qa.
