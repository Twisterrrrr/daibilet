...
2. **`significantSuburbs` - блок «Значимые пригороды {City_Род}»** на city hub **после** in-city mustSee (не в chips «Главные места»). Snap carousel: одна мини-локация на экран (swipe / arrows), nested POI внутринутри карточки. Copy: «Поездка на день рядом с городом - отдельные мини-локации и точки внутри них»; бейдж карточки - «Пригород» (один раз на карточке, не на каждой nested-строке). Nested POI - нумерованный `<ol>`; «В маршрут» у каждой точки (slug/match или suburb editorial stub). Для СПб: Петергоф, Царское Село, Кронштадт; Калининград: коса / Зеленоградск / Светлогорск / Балтийск / Янтарный.
...
1. **City hub** - mustSee + significantSuburbs + фильтры (структура «куда сходить»).
...
- Checkout hostnames / paths: **Path A** thin на `daibilet.ru/checkout/admissions/*` (Cursor, NOW); **Path B** calc scaffold `daibilet.ru/checkout/calc` (FUTURE); **Codex parallel** thin на `pay.daibilet.ru` (не merge поверх); optional alias `checkout.daibilet.ru` - см. qa.md; также `supplier.daibilet.ru`, `finance-api.daibilet.ru`.
