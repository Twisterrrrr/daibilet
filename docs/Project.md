...
| 3 | **AI / статьи и city hubs** — редакционный контент и SEO-якоря; **Region Hub IA** для `type=region` ([region-hub-v1.md](./region-hub-v1.md), live tier [region-live-tier.md](./region-live-tier.md)); admin AI-рерайт описаний событий → `EventOverride` ([event-description-ai-rewrite.md](./event-description-ai-rewrite.md)) | поддерживающий поток |
...
| Карточка в индексе `/cities` | **адмцентры субъекта** (Владикавказ, Ханты-Мансийск, Самара, …): events ≥ 1. **Региональные городки** (Тольятти, Сургут, Новокузнецк, Сортавала): **events ≥ 3**, иначе свёртка в субъект через `cityToRegion`. Прямой `/cities/{slug}` не прячем |
...
| Город области без отдельного хаба | `cityToRegion` → субъект; H1/лид формулы A на `/cities/{region}?city={slug}` (Выборг → `/cities/leningradskaya-oblast?city=vyborg`). Поиск `/cities` и шапка: `matchSearchGeoHits`. Palace suburbs СПб → `?suburb=#city-suburbs` |
...
- Nav: **V1.2 (owner 2026-08-24):** primary **Афиша · Город · Подборки · Блог**. Афиша = `/events` (city-aware). Город = `/cities/{slug}` при выбранном городе, иначе `/cities`. **Места** (`/places`) убраны из primary; остаются footer / secondary / SEO. Каталог `/places` без изменений chrome (eyebrow, H1, теги, 301 `/venues`/`/locations` листингов, entity URL канон). **Мой день** - смешанный поиск в шапке подбора. `UX.LOC3` long rename superseded.
...
- **Catalog hub canonical (owner 2026-08-14):** query на `/places` `/events` `/cities` **strip** → абсолютный хаб (`https://daibilet.ru/places` и т.д.). **Никогда** не канонизировать каталог на `/`. Sitemap listing = `/places` без `?city=`/`?family=`/`?category=` фасетов. `/podborki/c/{seoPilot}` - self-canonical; soft `/podborki?city=` meta-pilot → 301 на маркер; moscow leftover Meta на `/podborki/c/moscow` (noindex).
- **Подборки city SEO (2026-08-11 финал; Phase 1 marker 2026-09-03):** active пилот **kaliningrad + saint-petersburg** (moscow Meta leftover harmless); unique Title/Desc/H1 + **self-canonical** на `/podborki/c/{seoSlug}`; soft `?city=` → 301; Groups A/B не ломать; C MULTI + E - stable index (не мигать ≥6 при events>0); D `salute-9-may` - 200+index круглый год. `SeoOverride` + templates (2026-08-12). **Пилот-2 (план):** `nizhny-novgorod` + `perm` после закрепления КГД+СПб в Вебмастере (`SEO.PODBORKI-PILOT-2`) - подхватывается той же маркерной машиной. План: [seo-podborki-chpu-plan.md](./seo-podborki-chpu-plan.md).
