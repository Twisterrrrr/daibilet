...
- **Подборки city SEO (2026-08-11):** пилот Meta на soft `/podborki?city=` (kaliningrad / saint-petersburg / moscow) + **self-canonical**; H1 идейный хаб (не афиша `/cities`). Slug Meta = SEO path-канон (`normalizeKnownCitySlug`), не DB translit. Маркерный ЧПУ `/podborki/c/{city}` - следующий спринт. План: [seo-podborki-chpu-plan.md](./seo-podborki-chpu-plan.md).
...
- **Thin listing:** &lt; 6 офферов → `noindex,follow`, **кроме** category×city с exact editorial в `seo-listing-texts` (тогда `index,follow` при ≥1 оффере; 0 = noindex). При ровно 6–7 офферах - доп. карточки смежных категорий (`LandingThinRelatedCards`).
